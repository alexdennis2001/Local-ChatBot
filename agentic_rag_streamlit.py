...
# define the assistant's role and language
...
# insert the system message at the beginning of the prompt
...
user_question = st.chat_input("Hola soy el ChatBot asistente de Ing. Dennis. ¿En qué puedo ayudarte hoy?")
...
if user_question:
    ...
    with st.chat_message("assistant"):
        # Detectar bloques de LaTeX con \[...\] o $$...$$
        latex_blocks = re.split(r"(\\\[.*?\\\]|\$\$.*?\$\$)", ai_message, flags=re.DOTALL)

        for block in latex_blocks:
            if block.startswith("\\[") and block.endswith("\\]"):
                st.latex(block[2:-2])  # elimina delimitadores \[ \]
